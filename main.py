...
def _limpiar_desc(texto: Optional[str]) -> str:
    ...
    t = _re.sub(
        r"(?:\s|[-—,.:])*\b[A-ZÁÉÍÓÚÑ][A-Za-zÁÉÍÓÚÑ&\-/]{1,}\s*$",
        "",
        t,
    ).strip()
    t = t.strip(" .,-–—")
...
def _upsert_png_map_row(
    ...
    if exists.any():
        ...
        df.at[idx, "precio_unitario"] = (
            pu if pu is not None else df.at[idx, "precio_unitario"]
        )
        df.at[idx, "precio_total"] = (
            total if total is not None else df.at[idx, "precio_total"]
        )
    else:
        df = pd.concat(
            [
...
@app.get("/")
def root():
    return {"service": "tinnova-api", "ok": True}
...
def auto_build_png_map():
    if not os.path.isfile(COTIZACIONES_CSV):
        raise HTTPException(
            status_code=400,
            detail="No existe cotizaciones.csv",
        )
    ...
    if "archivo" not in df.columns or "pagina" not in df.columns:
        raise HTTPException(
            status_code=400,
            detail="csv requiere archivo & pagina",
        )
    rows = []
    df["_f"] = pd.to_datetime(
        df["fecha"],
        dayfirst=True,
        errors="coerce",
    )
    for (archivo, pagina), sub in df.groupby(["archivo", "pagina"]):
        ...
        fn = (
            f"{base}_p{pagina}_xref.png"
            if pagina
            else f"{base}_xref.png"
        )
        ...
        pu = (
            sub["precio_unitario"].median()
            if "precio_unitario" in sub.columns
            else None
        )
        pt = (
            sub["precio_total"].median()
            if "precio_total" in sub.columns
            else None
        )
        rows.append(
            {
                "filename_png": fn,
                "archivo_pdf": os.path.basename(archivo),
                "pagina": pagina,
                "descripcion": desc,
                "cantidad": cantidad,
                "precio_unitario": pu,
...
def set_png_map(item: PngMapUpsert):
    _upsert_png_map_row(
        ...
        if item.precio_total is not None
        else None,
...
async def buscar_similares_imagen(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail="No se pudo abrir imagen",
        )
    try:
        if _embeddings is None or len(_filenames) == 0:
            _load_base_visual()
    ...
    for it in candidatos:
        if it.similitud < float(min_sim):
            continue
        row = _map_row(it.filename)
        if only_mapped and row is None:
            continue
        desc = _strict_desc(it.filename) or ""
        pu = _strict_pu(it.filename)
        it.descripcion_sugerida = desc
        it.precio_unitario_estimado = (
            None if pu is None else round(float(pu), 2)
        )
        items.append(it)
        if len(items) >= top_k:
            break
...
async def cotizar_desde_seleccion(
    ...
    if row is None:
        raise HTTPException(
            status_code=422,
            detail="PNG no está en mapa",
        )
    desc_in = (descripcion or "").strip()
    desc = (
        _limpiar_desc(desc_in)
        if (AUTO_SANITIZE_DESC and desc_in)
        else (desc_in or (_strict_desc(filename) or ""))
    )
    if precio_unitario_override is not None:
        try:
            ...
            raise HTTPException(
                status_code=422,
                detail="Override inválido",
            )
        if pu < 0:
            raise HTTPException(
                status_code=422,
                detail="Override negativo",
            )
    else:
    ...
    return CotizarResponse(
        ...
        resultado=ResultadoCotizacion(
            precio_unitario_estimado=float(round(pu, 4)),
...
async def generar_proforma(payload: ProformaPayload = Body(...)):
    ...
    for it in payload.items:
        ...
        if it.is_custom:
            if not it.custom_filename:
                raise HTTPException(
                    status_code=422,
                    detail=f"{it.filename}: falta custom_filename",
                )
            if it.precio_unitario_override is None:
                raise HTTPException(
                    status_code=422,
                    detail=f"{it.filename}: falta precio",
                )
            try:
                ...
                raise HTTPException(
                    status_code=422,
                    detail=f"{it.filename}: override inválido",
                )
            if pu < 0:
                raise HTTPException(
                    status_code=422,
                    detail=f"{it.filename}: override negativo",
                )
            desc = (
                _limpiar_desc((it.descripcion or "").strip())
                if AUTO_SANITIZE_DESC
                else (it.descripcion or "")
            )
            try:
                _append_image_to_base_visual(
                    os.path.basename(it.custom_filename)
                )
            except Exception:
                pass
        ...
        if row is None:
            raise HTTPException(
                status_code=422,
                detail=f"{it.filename} sin mapa",
            )
        desc_in = (it.descripcion or "").strip()
        desc = (
            _limpiar_desc(desc_in)
            if (AUTO_SANITIZE_DESC and desc_in)
            else (desc_in or (_strict_desc(it.filename) or ""))
        )
    ...
    if os.path.exists(COTIZACIONES_CSV):
        ...
        for c in cot_cols:
            if c not in df_old.columns:
                df_old[c] = None
        df = pd.concat(
            [df_old, pd.DataFrame(cot_rows, columns=cot_cols)],
            ignore_index=True,
        )
    ...
    cliente_id = sb_get_or_create_cliente(
        payload.datos.cliente,
        payload.datos.contacto,
        payload.datos.ruc,
        payload.datos.direccion,
    )
    sb_insert_proforma(
        cliente_id,
        numero,
...
def _crear_pdf(
    ...
    datos_tbl = [
        ...
        [
            ...
            str(datos.validez_oferta_dias or 15),
        ],
    ...
    for it in items:
        ...
        rows.append(
            [
                img,
                Paragraph(it.descripcion, p_desc),
                str(it.cantidad),
                f"{moneda} {it.precio_unitario:.2f}",
                f"{moneda} {it.total:.2f}",
            ]
        )
    t2 = Table(
        head + rows,
        colWidths=[38 * mm, 270, 45, 60, 65],
        repeatRows=1,
    )
    t2.setStyle(
        TableStyle(
    ...
    t3 = Table(
        [
            ...
            [
                f"IGV ({igv_pct:.0f}%):",
                Paragraph(f"{moneda} {igv:.2f}", p_right),
            ],
            [
                "Total:",
                Paragraph(
...
async def import_items_csv(
    ...
    auto_build_map: bool = Query(True),
):
