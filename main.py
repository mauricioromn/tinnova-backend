...
if not logger.handlers:
    logging.basicConfig(level=logging.INFO)  # usar DEBUG si deseas más detalle
...
class ProformaDatos(BaseModel):
    ...
    fecha: Optional[str] = Field(None, description="DD/MM/YYYY")
...
def _append_image_to_base_visual(filename_png: str):
    ...
    if base in set(_filenames or []):
        return
    ruta = os.path.join(STATIC_DIR, base)
    if not os.path.exists(ruta):
        return
    with Image.open(ruta) as im:
        vec = _embed_image(im)  # (512,)
    ...
    if _embeddings is None or _embeddings.shape[0] == 0:
        _filenames = [base]
        _embeddings = np.array([vec], dtype=np.float32)
    else:
...
def _leer_png_map() -> Optional[pd.DataFrame]:
    ...
    for c in req:
        if c not in df.columns:
            raise ValueError("png_map.csv debe tener columnas: " + ", ".join(req))
...
def _upsert_png_map_row(filename_png: str, descripcion: str, cantidad: int, pu: float, total: float, archivo_pdf: Optional[str]):
...
# ========= Subir imagen custom (con embedding) — con logging ==========
...
def auto_build_png_map():
    if not os.path.isfile(COTIZACIONES_CSV):
        raise HTTPException(status_code=400, detail="No existe cotizaciones.csv para autoconstruir.")
    ...
    if "archivo" not in df.columns or "pagina" not in df.columns:
        raise HTTPException(status_code=400, detail="cotizaciones.csv debe tener 'archivo' y 'pagina'")
    rows = []
    df["_fecha_ord"] = pd.to_datetime(df["fecha"], dayfirst=True, errors="coerce")
    for (archivo, pagina), sub in df.groupby(["archivo","pagina"]):
        archivo = str(archivo)
        pagina  = int(pagina) if not pd.isna(pagina) else None
        base = re.sub(r"\.pdf$","", os.path.basename(archivo))
        fn = f"{base}_p{pagina}_xref.png" if pagina else f"{base}_xref.png"
        sub_sorted = sub.sort_values("_fecha_ord", ascending=False)
        desc = ""
...
def set_png_map(item: PngMapUpsert):
    _upsert_png_map_row(
        filename_png=item.filename_png,
...
async def debug_precio(request: Request, filename: Optional[str] = Query(None)):
    if (not filename) and request.method == "POST":
        try:
            data = await request.json(); filename = data.get("filename")
        except: pass
    if not filename:
        raise HTTPException(status_code=422, detail="Falta 'filename'")
...
async def buscar_similares_imagen(
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail="No se pudo abrir la imagen.")
    try:
        if _embeddings is None or len(_filenames) == 0:
            _load_base_visual()
    ...
    for it in candidatos:
        ...
        if RETURN_ONLY_MAPPED and (row is None):
            continue
...
async def cotizar_desde_seleccion(
    ...
    if cantidad <= 0:
        raise HTTPException(status_code=422, detail="La cantidad debe ser > 0.")

    row = _map_row(filename)
    if row is None:
        raise HTTPException(status_code=422, detail="El PNG no está en png_map.csv.")

    desc_in = (descripcion or "").strip()
    if desc_in:
        desc = _limpiar_desc(desc_in) if AUTO_SANITIZE_DESC else desc_in
    else:
        desc = _strict_desc(filename) or ""

    if precio_unitario_override is not None:
        try:
            pu = float(precio_unitario_override)
        except:
            raise HTTPException(status_code=422, detail="Override inválido")
        if pu < 0:
            raise HTTPException(status_code=422, detail="Override no puede ser negativo")
    else:
        pu = _strict_pu(filename)
        if pu is None:
            raise HTTPException(status_code=422, detail="Sin precio en mapa (completa precio_unitario o total/cantidad).")
        pu = float(pu)
...
async def generar_proforma(payload: ProformaPayload = Body(...)):
    if not payload.items:
        raise HTTPException(status_code=422, detail="Debes enviar al menos un item.")
    if not payload.datos.cliente:
    ...
    resumen_items: List[ProformaResumenItem] = []
    subtotal = 0.0

    for it in payload.items:
        # A) custom
        if it.is_custom:
            if not it.custom_filename:
                raise HTTPException(status_code=422, detail=f"{it.filename}: falta custom_filename")
            if it.precio_unitario_override is None:
                raise HTTPException(status_code=422, detail=f"{it.filename}: falta precio_unitario_override en item custom")
            try:
                pu = float(it.precio_unitario_override)
            except:
                raise HTTPException(status_code=422, detail=f"{it.filename}: override inválido")
            if pu < 0:
                raise HTTPException(status_code=422, detail=f"{it.filename}: override no puede ser negativo")
            ...
            resumen_items.append(ProformaResumenItem(
                filename=os.path.basename(it.custom_filename),
                cantidad=it.cantidad,
                descripcion=desc,
                precio_unitario=float(round(pu,4)),
                total=total_linea
            ))
            continue
        ...
        row = _map_row(it.filename)
        if row is None:
            raise HTTPException(status_code=422, detail=f"{it.filename}: no está en png_map.csv")
        ...
        if it.precio_unitario_override is not None:
            try:
                ...
                raise HTTPException(status_code=422, detail=f"{it.filename}: override inválido")
            if pu < 0:
                raise HTTPException(status_code=422, detail=f"{it.filename}: precio_unitario_override no puede ser negativo")
        else:
            pu_map = _strict_pu(it.filename)
            if pu_map is None:
                raise HTTPException(status_code=422, detail=f"{it.filename}: sin precio en mapa y sin override")
            pu = float(pu_map)
        ...
        resumen_items.append(ProformaResumenItem(
            ...
            descripcion=desc, precio_unitario=float(round(pu,4)),
            total=total_linea
        ))

    igv = round(subtotal * (igv_pct/100.0), 2)
    ...
    _crear_pdf(
        pdf_path=pdf_path, numero=numero, fecha=fecha_str,
        datos=payload.datos, items=resumen_items,
        subtotal=subtotal, igv=igv, total=total, moneda=moneda, igv_pct=igv_pct
    )
    ...
    if os.path.exists(COTIZACIONES_CSV):
        ...
        df_add = pd.DataFrame(cot_rows, columns=cot_cols)
        df = pd.concat([df_old, df_add], ignore_index=True)
        df.to_csv(COTIZACIONES_CSV, index=False, encoding="utf-8")
    else:
        pd.DataFrame(cot_rows, columns=cot_cols).to_csv(COTIZACIONES_CSV, index=False, encoding="utf-8")

    # 4) Upsert png_map.csv
    for it in resumen_items:
        _upsert_png_map_row(
            filename_png=os.path.basename(it.filename),
            descripcion=it.descripcion,
    ...
    # 5) Registrar en Supabase (si hay config)
    try:
    ...
    cliente_id = sb_get_or_create_cliente(
        nombre=payload.datos.cliente,
        contacto=payload.datos.contacto,
        ruc=payload.datos.ruc,
...
def _crear_pdf(pdf_path: str, numero: str, fecha: str, datos: ProformaDatos,
               items: List[ProformaResumenItem], subtotal: float, igv: float, total: float,
               moneda: str, igv_pct: float):
    ...
    doc = SimpleDocTemplate(
        pdf_path, pagesize=A4, rightMargin=24, leftMargin=24, topMargin=24, bottomMargin=24,
        title=f"Proforma {numero}"
    )
    ...
    empresa = [
        f"<b>{COMPANY_NAME}</b>", COMPANY_RUC, COMPANY_ADDRESS,
    ...
    datos_tbl = [
        ...
        ["Validez de oferta (días):", str(datos.validez_oferta_dias or 15)],
    ...
    rows = []
    for it in items:
        img = _flow_img(it.filename, max_lado_mm=35.0)
    ...
    def _footer(canvas, doc):
        ...
        w,h = _A4
        canvas.setStrokeColor(colors.grey); canvas.setLineWidth(0.5)
...
async def import_items_csv(request: Request, file: UploadFile = File(...), auto_build_map: bool = Query(True)):
    ...
    if not file.filename.lower().endswith(".csv"):
        raise HTTPException(status_code=422, detail="Sube un .csv")
...
async def import_items_json(request: Request, items: List[dict] = Body(...), auto_build_map: bool = Query(True)):
    ...
    if not isinstance(items, list) or len(items) == 0:
        raise HTTPException(status_code=422, detail="Envía una lista de objetos")
